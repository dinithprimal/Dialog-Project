<?php
    ...
    $mail->Body   .= ''.$userFullName.' has requested to transfer the following risk <em>'.$indexRisk.'</em> from divisional risk registry to the ERM.<br>';
    ...
    $queryActivity = "INSERT INTO divactivity (indexRisk, empNo, Date, Time, Des) VALUES ('$indexRisk', '$empNo', '$Date', '$Time', 'Requested to transfer the risk from Divisional Risk Registry to ERM')";
    if($conn->query($queryActivity) === TRUE){
        $queryUpdateTrindex = "UPDATE divtrindex SET status = 'Pending', moveRequest = '$empNo' WHERE indexRisk = '$indexRisk'";
