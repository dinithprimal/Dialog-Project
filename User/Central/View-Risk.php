<?php
    ...
    $validate = "SELECT COUNT(*) AS tot FROM trindex WHERE indexRisk = '$riskIndex'";
    ...
    if(isset($_SESSION['user'])){
        ...
        $queryUserDetails = "SELECT idUser, fullName, empNo, role, category FROM user_details WHERE username = '$username'";
...
<html lang="en">
    ...
    <body>
        ...
        <?php
            $riks_sql = "SELECT * FROM risk WHERE indexRisk = '$riskIndex'";
            ...
            $userCategory = $userDetails['category'];

            $sqlRiskReporter = "SELECT COUNT(*) AS repTot FROM risk WHERE indexRisk = '$riskIndex' AND reporter = '$empNo'";
            $resultRiskReporter = mysqli_query($conn,$sqlRiskReporter) or die(mysqli_error($conn));
            $riskReporter = mysqli_fetch_array($resultRiskReporter);
            $riskReporter = $riskReporter['repTot'];

            $sqlRiskOwnership = "SELECT COUNT(*) AS ownTot FROM riskowners WHERE indexRisk = '$riskIndex' AND empNo = '$empNo'";
            $resultRiskOwnership = mysqli_query($conn,$sqlRiskOwnership) or die(mysqli_error($conn));
            $riskOwnership = mysqli_fetch_array($resultRiskOwnership);
            $riskOwnership = $riskOwnership['ownTot'];

            $sqlActionOwnership = "SELECT COUNT(*) AS actTot FROM actionowners WHERE indexRisk = '$riskIndex' AND empNo = '$empNo'";
            $resultActionOwnership = mysqli_query($conn,$sqlActionOwnership) or die(mysqli_error($conn));
            $actionOwnership = mysqli_fetch_array($resultActionOwnership);
            $actionOwnership = $actionOwnership['actTot'];

            $activeQuery = "SELECT status FROM trindex WHERE indexRisk = '$riskIndex'";
                        ...
                        <div class="row mt-2 px-1">
                            <div class="table-responsive d-flex" style="height: 153px; overflow: auto; overflow-x: auto; overflow-y: auto;">
                                <table class="table table-sm table-striped table-hover">
                                    ...
                                    <tbody>
                                        <?php
                                            $sql_riskOwners = "SELECT fullName, empNo FROM user_details WHERE category = 'Head' AND empNo NOT IN (SELECT empNo FROM riskowners WHERE indexRisk = '$riskIndex') ORDER BY fullName";
                        ...
                        <div class="row mt-2 px-1">
                            <div class="table-responsive d-flex" style="height: 153px; overflow: auto; overflow-x: auto; overflow-y: auto;">
                                <table class="table table-sm table-striped table-hover">
                                    ...
                                    <tbody>
                                        <?php
                                            $sql_actionOwners = "SELECT fullName, empNo FROM user_details WHERE empNo NOT IN (SELECT empNo FROM actionowners WHERE indexRisk = '$riskIndex') ORDER BY fullName";
        ...
        <div class="modal fade" id="Close-Risk" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-sm btn-secondary" data-bs-dismiss="modal">Cancel</button>
                        <button type="button" id="closeRisk" name="closeRisk" class="btn btn-sm btn-danger" disabled>Request</button>
                    </div>
                </div>
            </div>
        </div>
                    ...
                    <div class="modal-body" style="width:auto;">
                        <div class="container">
                            <table class="table table-fixed table-fixedheader table-striped table-hover table-sm">
                                <thead>
                                    <tr>
                                        <th style="width:12%"><small><small>Date / Time</small></small></th>
                                        <th style="width:76%"><small><small>Description</small></small></th>
                                        <th style="width:12%"><small><small>Task By</small></small></th>
                                    </tr>
                                </thead>
                                <tbody style="overflow-y:auto;">
                                <?php
                                    $sqlLog = "SELECT * FROM activity WHERE indexRisk = '$riskIndex' ORDER BY idActivity DESC";
                            ...
                            <div class="container">
                                <h4 align="center">
                                    <small>
                                        ...
                                        <?php
                                            }else if($risk_details['closeStatus'] == 2){
                                        ?>
                                            <span class="text-danger"><b> (Requested to close this risk)</b></span>
                                        <?php
                                            }
                                        ?>
                                    </small>
                                    ...
                                    <span class="text-danger"><small><small><small>
                                        <?php
                                            if($activeStatus == "Inactive"){
                                                $querySelDivID = "SELECT DIVindexRisk FROM moverisk WHERE ERMindexRisk = '$riskIndex'";
                                                $resultSelDivID = mysqli_query($conn,$querySelDivID) or die(mysqli_error($conn));
                                                $selDivID = mysqli_fetch_array($resultSelDivID);
                                                $divID = $selDivID['DIVindexRisk'];
                                                echo    "Risk transferred from ERM to divisional risk registry - Divisional Risk ID: ".$divID."";
                                            }else if($activeStatus == "Pending"){
                                                echo    "This risk requested to transfer to divisional risk registry and await for admin approval";
                                            }
                                        ?>
                                    </small></small></small>
                                </h4>
                                                        ...
                                                        <div class="col mb-4 mb-lg-1">
                                                            <div class="row justify-content-between align-item-center">
                                                                <div class="col align-item-center">
                                                                    <button style="float: right" type="button" data-bs-toggle="modal" data-bs-target="#ReOpen" class="btn btn-sm btn-info mx-2" <?php if(($riskReporter == 0) & ($riskOwnership == 0)){   echo    "disabled";  }  ?>>Open Again</button>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                                ...
                                ?>
                                <?php
                                    $sqlInfoProg = "SELECT * FROM infogather WHERE status = 0";
                                                                ...
                                                                <div class="row justify-content-between align-item-center">
                                                                    <div class="col align-item-center">
                                                                        <?php
                                                                            ...
                                                                            $sqlUpdate = "SELECT * FROM infogatherdetails WHERE idInfo = '$infoProgID' AND indexRisk = '$riskIndex' AND empNo = '$userEmpNo'";
                                                                            ...
                                                                            $queryInfoRecord = "SELECT COUNT(*) AS activityTot FROM activity WHERE (indexRisk = '$riskIndex') AND des NOT IN ('Newly Added','New Action Owner','Remove Action Owner','Add New Risk Owner','Remove Risk Owner') AND (empNo = '$userEmpNo') AND ((Date > '$startDate' OR (Date = '$startDate' AND Time > '$startTime')))";
                                                                            ...
                                                                            $sqlDeadline = "SELECT deadline FROM risktreatmentdeadline WHERE id = (SELECT MAX(id) FROM risktreatmentdeadline WHERE indexRisk = '$riskIndex')";
                                ...
                                <div class="row row-cols-1 g-1 g-lg-2 mb-2">
                                    <div class="col">
                                        <button type="button" data-bs-toggle="modal" data-bs-target="#View-Risk-Log" class="btn btn-sm btn-primary mx-2" style="float: right;">View Log</button>
                                        <?php
                                            if($risk_details['closeStatus'] == 0){
                                                if($riskReporter == 0){
                                                    if($userCategory == "Head"){
                                                        if(($riskOwnership == 0)){
                                        ?>
                                            <button type="button" id="addMeRiskOwner" name="addMeRiskOwner" class="btn btn-sm btn-primary mx-2" style="float: right;" <?php if($activeStatus != "Active"){  echo    "disabled"; }  ?>>Add Me as a Risk Owner</button>
                                        <?php
                                                        }
                                                    }
                                                }
                                            ...
                                            if($activeStatus != "Inactive"){
                                        ?>
                                            <button type="button" id="requestSendDivision" name="requestSendDivision" class="btn btn-sm btn-primary mx-2" style="float: right;" <?php if((($riskReporter == 0) & ($riskOwnership == 0) & ($actionOwnership == 0)) | ($activeStatus != "Active")){   echo    "disabled";  }  ?>><?php if($activeStatus == "Active"){ echo    "Request Send this Risk to Divisional Risk Registry";  }else if($activeStatus == "Pending"){  echo    "Requested Send this Risk to Divisional Risk Registry";  } ?></button>
                                        <?php
                                                        ...
                                                        <div class="col mb-4 mb-lg-1">
                                                            <div class="form-control">
                                                                <div class="row d-flex justify-content-between align-items-center">
                                                                    ...
                                                                    <div class="col-6">
                                                                        <label><small><small><b><?php  echo    $riskIndex;   ?></b></small></small></label>
                                                                    </div>
                                                                </div>
                                                                ...
                                                                <div class="row d-flex justify-content-between align-items-center mt-2">
                                                                    ...
                                                                    <div class="col-6">
                                                                        <?php
                                                                            $sql_LastUpdate = "SELECT empNo,date,time FROM lastupdate WHERE indexRisk = '$riskIndex'";
                                                        ...
                                                        <div class="col mb-4 mb-lg-1">
                                                            <div class="form-control">
                                                                ...
                                                                <?php
                                                                    if($risk_details['closeStatus'] == 0){
                                                                ?>
                                                                    <button type="button" data-bs-toggle="modal" data-bs-target="#risk-owners" class="btn btn-info m-auto mt-2" style="padding: .25rem .4rem; font-size: .875rem; line-height: .95; border-radius: .2rem; width: 50%;"<?php if((($riskReporter == 0) & ($riskOwnership == 0)) | ($activeStatus != "Active")){   echo    "disabled";  }  ?>>Add Risk Owner</button>
                                                                <?php
                                                                    }
                                                                ?>
                                                                </div>
                                                                <div class="row mt-2 px-1">
                                                                    <div class="table-responsive d-flex" style="height: 153px; overflow: auto; overflow-x: auto; overflow-y: auto;">
                                                                        <table class="table table-sm table-striped table-hover">
                                                                            ...
                                                                            <tbody>
                                                                                <?php
                                                                                    $sql_riskOwners = "SELECT fullName, empNo FROM user_details WHERE empNo IN (SELECT empNo FROM riskowners WHERE indexRisk = '$riskIndex') ORDER BY fullName";
                                                                                    $resul_riskOwners = mysqli_query($conn,$sql_riskOwners) or die(mysqli_error($conn));
                                                                                    $count = mysqli_num_rows($resul_riskOwners);
                                                                                    if($count>0){
                                                                                        while($riskOwners = mysqli_fetch_array($resul_riskOwners)){
                                                                                ...
                                                                                <tr>
                                                                                    ...
                                                                                    <?php
                                                                                        if($risk_details['closeStatus'] == 0){
                                                                                    ?>
                                                                                    <td align="center"><small><small><button type="button" name="riskOwnerButton" onclick="removeRiskOwners(this);" id="<?php  echo    $riskOwners['empNo'];   ?>" class="btn btn-sm btn-danger" style="padding: .20rem .4rem; font-size: .800rem; line-height: .95; border-radius: .2rem;" <?php if((($riskReporter == 0) & ($riskOwnership == 0)) | ($activeStatus != "Active")){   echo    "disabled";  }  ?>>Remove</button></small></small></td>
                                                                                    <?php
                                                                                        }else{
                                                                                    ?>
                                                                                        <td></td>
                                                        ...
                                                        <div class="col mb-4 mb-lg-1">
                                                            <div class="form-control">
                                                                ...
                                                                <?php
                                                                    if($risk_details['closeStatus'] == 0){
                                                                ?>
                                                                    <button type="button" data-bs-toggle="modal" data-bs-target="#action-owners" class="btn btn-info m-auto mt-2" style="padding: .25rem .4rem; font-size: .875rem; line-height: .95; border-radius: .2rem; width: 50%;" <?php if((($riskReporter == 0) & ($riskOwnership == 0) & ($actionOwnership == 0)) | ($activeStatus != "Active")){   echo    "disabled";  }  ?>>Add Action Owner</button>
                                                                <?php
                                                                    }
                                                                ?>
                                                                </div>
                                                                <div class="row mt-2 px-1">
                                                                    <div class="table-responsive d-flex" style="height: 153px; overflow: auto; overflow-x: auto; overflow-y: auto;">
                                                                        <table class="table table-sm table-striped table-hover">
                                                                            ...
                                                                            <tbody>
                                                                                <?php
                                                                                    $sql_actionOwners = "SELECT fullName, empNo FROM user_details WHERE empNo IN (SELECT empNo FROM actionowners WHERE indexRisk = '$riskIndex') ORDER BY fullName";
                                                                                    $resul_actionOwners = mysqli_query($conn,$sql_actionOwners) or die(mysqli_error($conn));
                                        ...
                                        <div class="p-0">
                                            <div class="card" style="width: auto; height: auto;">
                                                ...
                                                <div class="card-body">
                                                    <div class="form-control p-3 mb-3">
                                                        ...
                                                        <div class="row d-flex justify-content-between align-items-center">
                                                            ...
                                                            <?php
                                                                if($risk_details['closeStatus'] == 0){
                                                            ?>
                                                                <button class="btn btn-sm btn-info" data-bs-toggle="modal" data-bs-target="#business-impact" type="button" style="padding: .25rem .4rem; font-size: .820rem; line-height: .95; border-radius: .2rem; display: inline;" <?php if((($riskReporter == 0) & ($riskOwnership == 0)) | ($activeStatus != "Active")){   echo    "disabled";  }  ?>>Edit</button>
                                                            <?php
                                                                }
                                                            ?>
                                                            </div>
                                                        ...
                                                        <div class="row d-flex justify-content-between align-items-center mt-3">
                                                            ...
                                                            <?php
                                                                if($risk_details['closeStatus'] == 0){
                                                            ?>
                                                                <button class="btn btn-sm btn-info" data-bs-toggle="modal" data-bs-target="#risk-description" type="button" style="padding: .25rem .4rem; font-size: .820rem; line-height: .95; border-radius: .2rem; display: inline;" <?php if((($riskReporter == 0) & ($riskOwnership == 0)) | ($activeStatus != "Active")){   echo    "disabled";  }  ?>>Edit</button>
                                                            <?php
                                                                }
                                                            ?>
                                                            </div>
                                                        ...
                                                        <div class="row d-flex justify-content-between align-items-center mt-3">
                                                            ...
                                                            <?php
                                                                if($risk_details['closeStatus'] == 0){
                                                            ?>
                                                                <button class="btn btn-sm btn-info" data-bs-toggle="modal" data-bs-target="#risk-category" type="button" style="padding: .25rem .4rem; font-size: .820rem; line-height: .95; border-radius: .2rem; display: inline;" <?php if((($riskReporter == 0) & ($riskOwnership == 0)) | ($activeStatus != "Active")){   echo    "disabled";  }  ?>>Edit</button>
                                                            <?php
                                                                }
                                                            ?>
                                                            </div>
                                                        ...
                                                        <div class="row d-flex justify-content-between align-items-center mt-3">
                                                            ...
                                                            <?php
                                                                if($risk_details['closeStatus'] == 0){
                                                            ?>
                                                                <button class="btn btn-sm btn-info" data-bs-toggle="modal" data-bs-target="#main-unit" type="button" style="padding: .25rem .4rem; font-size: .820rem; line-height: .95; border-radius: .2rem; display: inline;" <?php if((($riskReporter == 0) & ($riskOwnership == 0)) | ($activeStatus != "Active")){   echo    "disabled";  }  ?>>Edit</button>
                                                            <?php
                                                                }
                                                            ?>
                                                            </div>
                                                    ...
                                                    <div class="form-control p-3 mb-3">
                                                        ...
                                                        <div class="row d-flex justify-content-between align-items-center">
                                                            ...
                                                            <?php
                                                                if($risk_details['closeStatus'] == 0){
                                                            ?>
                                                                <button class="btn btn-sm btn-info" data-bs-toggle="modal" data-bs-target="#KRI" type="button" style="padding: .25rem .4rem; font-size: .820rem; line-height: .95; border-radius: .2rem; display: inline;" <?php if((($riskReporter == 0) & ($riskOwnership == 0)) | ($activeStatus != "Active")){   echo    "disabled";  }  ?>>Edit</button>
                                                            <?php
                                                                }
                                                            ?>
                                                            </div>
                                                        ...
                                                        <div class="row d-flex justify-content-between align-items-center mt-3">
                                                            ...
                                                            <?php
                                                                if($risk_details['closeStatus'] == 0){
                                                            ?>
                                                                <button class="btn btn-sm btn-info" data-bs-toggle="modal" data-bs-target="#BAU" type="button" style="padding: .25rem .4rem; font-size: .820rem; line-height: .95; border-radius: .2rem; display: inline;" <?php if((($riskReporter == 0) & ($riskOwnership == 0)) | ($activeStatus != "Active")){   echo    "disabled";  }  ?>>Edit</button>
                                                            <?php
                                                                }
                                                            ?>
                                                            </div>
                                                        ...
                                                        <div class="row d-flex justify-content-between align-items-center mt-3">
                                                            ...
                                                            <?php
                                                                if($risk_details['closeStatus'] == 0){
                                                            ?>
                                                                <button class="btn btn-sm btn-info" data-bs-toggle="modal" data-bs-target="#Risk-Alert" type="button" style="padding: .25rem .4rem; font-size: .820rem; line-height: .95; border-radius: .2rem; display: inline;" <?php if((($riskReporter == 0) & ($riskOwnership == 0)) | ($activeStatus != "Active")){   echo    "disabled";  }  ?>>Edit</button>
                                                            <?php
                                                                }
                                                            ?>
                                                            </div>
                                                        ...
                                                        <div class="row d-flex justify-content-between align-items-center mt-3">
                                                            ...
                                                            <?php
                                                                if($risk_details['closeStatus'] == 0){
                                                            ?>
                                                                <button class="btn btn-sm btn-info" data-bs-toggle="modal" data-bs-target="#Risk-Risk" type="button" style="padding: .25rem .4rem; font-size: .820rem; line-height: .95; border-radius: .2rem; display: inline;" <?php if((($riskReporter == 0) & ($riskOwnership == 0)) | ($activeStatus != "Active")){   echo    "disabled";  }  ?>>Edit</button>
                                                            <?php
                                                                }
                                                            ?>
                                                            </div>
                                                        ...
                                                        <div class="row d-flex justify-content-between align-items-center mt-3">
                                                            ...
                                                            <?php
                                                                if($risk_details['closeStatus'] == 0){
                                                            ?>
                                                                <button class="btn btn-sm btn-info" data-bs-toggle="modal" data-bs-target="#Risk-Cause" type="button" style="padding: .25rem .4rem; font-size: .820rem; line-height: .95; border-radius: .2rem; display: inline;"<?php if((($riskReporter == 0) & ($riskOwnership == 0)) | ($activeStatus != "Active")){   echo    "disabled";  }  ?>>Edit</button>
                                                            <?php
                                                                }
                                                            ?>
                                                            </div>
                                                        ...
                                                        <div class="row d-flex justify-content-between align-items-center mt-3">
                                                            ...
                                                            <?php
                                                                if($risk_details['closeStatus'] == 0){
                                                            ?>
                                                                <button class="btn btn-sm btn-info" data-bs-toggle="modal" data-bs-target="#Risk-Impact" type="button" style="padding: .25rem .4rem; font-size: .820rem; line-height: .95; border-radius: .2rem; display: inline;" <?php if((($riskReporter == 0) & ($riskOwnership == 0)) | ($activeStatus != "Active")){   echo    "disabled";  }  ?>>Edit</button>
                                                            <?php
                                                                }
                                                            ?>
                                                            </div>
                                                        ...
                                                        <div class="row d-flex justify-content-between align-items-center mt-3">
                                                            ...
                                                            <?php
                                                                if($risk_details['closeStatus'] == 0){
                                                            ?>
                                                                <button class="btn btn-sm btn-info" data-bs-toggle="modal" data-bs-target="#Risk-Likelyhood-Level" type="button" style="padding: .25rem .4rem; font-size: .820rem; line-height: .95; border-radius: .2rem; display: inline;" <?php if((($riskReporter == 0) & ($riskOwnership == 0)) | ($activeStatus != "Active")){   echo    "disabled";  }  ?>>Edit</button>
                                                            <?php
                                                                }
                                                            ?>
                                                            </div>
                                                        ...
                                                        <div class="row d-flex justify-content-between align-items-center mt-3">
                                                            ...
                                                            <?php
                                                                if($risk_details['closeStatus'] == 0){
                                                            ?>
                                                                <button class="btn btn-sm btn-info" data-bs-toggle="modal" data-bs-target="#Risk-Impact-Level" type="button" style="padding: .25rem .4rem; font-size: .820rem; line-height: .95; border-radius: .2rem; display: inline;" <?php if((($riskReporter == 0) & ($riskOwnership == 0)) | ($activeStatus != "Active")){   echo    "disabled";  }  ?>>Edit</button>
                                                            <?php
                                                                }
                                                            ?>
                                                            </div>
                                                    ...
                                                    <div class="form-control p-3 mb-3">
                                                        ...
                                                        <div class="row d-flex justify-content-between align-items-center">
                                                            ...
                                                            <?php
                                                                if($risk_details['closeStatus'] == 0){
                                                            ?>
                                                                <button class="btn btn-sm btn-info" data-bs-toggle="modal" data-bs-target="#Risk-Existing-Control" type="button" style="padding: .25rem .4rem; font-size: .820rem; line-height: .95; border-radius: .2rem; display: inline;" <?php if((($riskReporter == 0) & ($riskOwnership == 0)) | ($activeStatus != "Active")){   echo    "disabled";  }  ?>>Edit</button>
                                                            <?php
                                                                }
                                                            ?>
                                                            </div>
                                                        ...
                                                        <div class="row d-flex justify-content-between align-items-center mt-3">
                                                            ...
                                                            <?php
                                                                if($risk_details['closeStatus'] == 0){
                                                            ?>
                                                                <button class="btn btn-sm btn-info" data-bs-toggle="modal" data-bs-target="#Control-Effectiveness" type="button" style="padding: .25rem .4rem; font-size: .820rem; line-height: .95; border-radius: .2rem; display: inline;" <?php if((($riskReporter == 0) & ($riskOwnership == 0)) | ($activeStatus != "Active")){   echo    "disabled";  }  ?>>Edit</button>
                                                            <?php
                                                                }
                                                            ?>
                                                            </div>
                                                        ...
                                                        <div class="row d-flex justify-content-between align-items-center mt-3">
                                                            ...
                                                            <?php
                                                                if($risk_details['closeStatus'] == 0){
                                                            ?>
                                                                <button class="btn btn-sm btn-info" data-bs-toggle="modal" data-bs-target="#Risk-Evaluation-Likelyhood-Level" type="button" style="padding: .25rem .4rem; font-size: .820rem; line-height: .95; border-radius: .2rem; display: inline;" <?php if((($riskReporter == 0) & ($riskOwnership == 0)) | ($activeStatus != "Active")){   echo    "disabled";  }  ?>>Edit</button>
                                                            <?php
                                                                }
                                                            ?>
                                                            </div>
                                                        ...
                                                        <div class="row d-flex justify-content-between align-items-center mt-3">
                                                            ...
                                                            <?php
                                                                if($risk_details['closeStatus'] == 0){
                                                            ?>
                                                                <button class="btn btn-sm btn-info" data-bs-toggle="modal" data-bs-target="#Risk-Evaluation-Impact-Level" type="button" style="padding: .25rem .4rem; font-size: .820rem; line-height: .95; border-radius: .2rem; display: inline;" <?php if((($riskReporter == 0) & ($riskOwnership == 0)) | ($activeStatus != "Active")){   echo    "disabled";  }  ?>>Edit</button>
                                                            <?php
                                                                }
                                                            ?>
                                                            </div>
                                                    ...
                                                    <div class="form-control p-3 mb-3">
                                                        ...
                                                        <div class="row d-flex justify-content-between align-items-center">
                                                            ...
                                                            <?php
                                                                if($risk_details['closeStatus'] == 0){
                                                            ?>
                                                                <button class="btn btn-sm btn-info" data-bs-toggle="modal" data-bs-target="#Risk-Response" type="button" style="padding: .25rem .4rem; font-size: .820rem; line-height: .95; border-radius: .2rem; display: inline;" <?php if((($riskReporter == 0) & ($riskOwnership == 0)) | ($activeStatus != "Active")){   echo    "disabled";  }  ?>>Edit</button>
                                                            <?php
                                                                }
                                                            ?>
                                                            </div>
                                                            ...
                                                            <div class="col-8">
                                                                <div class="table-responsive d-flex" style="height: 200px; overflow: auto; overflow-x: auto; overflow-y: auto;">
                                                                    <table class="table table-sm table-hover">
                                                                        <thead>
                                                                            <tr>
                                                                                <th scope="col" style="width: 60%;"><small><small>Activity</small></small></th>
                                                                                <th scope="col" style="width: 20%;"><small><small>Name</small></small></th>
                                                                                <th scope="col" style="width: 20%;"><small><small>Date / Time</small></small></th>
                                                                            </tr>
                                                                        </thead>
                                                                        <tbody>
                                                                            <?php
                                                                                $sql_activity = "SELECT activity, empNo, date, time FROM risktreatmentactivity WHERE indexRisk = '$riskIndex' ORDER BY id DESC";
                                                                ...
                                                                <?php
                                                                    if($risk_details['closeStatus'] == 0){
                                                                ?>
                                                                <button data-bs-toggle="modal" data-bs-target="#Add-Risk-Activity" type="button" style="padding: .35rem .5rem; font-size: .900rem; line-height: .95; border-radius: .2rem; float: right;" class="btn btn-info" <?php if((($riskReporter == 0) & ($riskOwnership == 0) & ($actionOwnership == 0)) | ($activeStatus != "Active")){   echo    "disabled";  }  ?>>Add Activity</button>
                                                                <?php
                                                                    }
                                                                ?>
                                                            </div>
                                                        ...
                                                        <div class="row d-flex justify-content-between align-items-center mt-3">
                                                            ...
                                                            <div class="table-responsive d-flex" style="height: 200px; overflow: auto; overflow-x: auto; overflow-y: auto;">
                                                                    <table class="table table-sm table-hover">
                                                                        <thead>
                                                                            <tr>
                                                                                <th scope="col" style="width: 60%;"><small><small>Deadline</small></small></th>
                                                                                <th scope="col" style="width: 20%;"><small><small>Name</small></small></th>
                                                                                <th scope="col" style="width: 20%;"><small><small>Date / Time</small></small></th>
                                                                            </tr>
                                                                        </thead>
                                                                        <tbody>
                                                                            <?php
                                                                                $sql_deadline = "SELECT deadline, empNo, date, time FROM risktreatmentdeadline WHERE indexRisk = '$riskIndex' ORDER BY id DESC";
                                                                ...
                                                                <?php
                                                                    if($risk_details['closeStatus'] == 0){
                                                                ?>
                                                                <button data-bs-toggle="modal" data-bs-target="#Add-Risk-Deadline" type="button" style="padding: .35rem .5rem; font-size: .900rem; line-height: .95; border-radius: .2rem; float: right;" class="btn btn-info" <?php if((($riskReporter == 0) & ($riskOwnership == 0) & ($actionOwnership == 0)) | ($activeStatus != "Active")){   echo    "disabled";  }  ?>>Change Deadline</button>
                                                                <?php
                                                                    }
                                                                ?>
                                                            </div>
                                                        ...
                                                        <div class="row d-flex justify-content-between align-items-center mt-3">
                                                            ...
                                                            <?php
                                                                if($risk_details['closeStatus'] == 0){
                                                            ?>
                                                                <button class="btn btn-sm btn-info" data-bs-toggle="modal" data-bs-target="#Mitigation-Effectiveness" type="button" style="padding: .25rem .4rem; font-size: .820rem; line-height: .95; border-radius: .2rem; display: inline;" <?php if((($riskReporter == 0) & ($riskOwnership == 0) & ($actionOwnership == 0)) | ($activeStatus != "Active")){   echo    "disabled";  }  ?>>Edit</button>
                                                            <?php
                                                                }
                                                            ?>
                                                            </div>
                                                        ...
                                                        <div class="row d-flex justify-content-between align-items-center mt-3">
                                                            ...
                                                            <?php
                                                                if($risk_details['closeStatus'] == 0){
                                                            ?>
                                                                <button class="btn btn-sm btn-info" data-bs-toggle="modal" data-bs-target="#Current-Status" type="button" style="padding: .25rem .4rem; font-size: .820rem; line-height: .95; border-radius: .2rem; display: inline;" <?php if((($riskReporter == 0) & ($riskOwnership == 0) & ($actionOwnership == 0)) | ($activeStatus != "Active")){   echo    "disabled";  }  ?>>Edit</button>
                                                            <?php
                                                                }
                                                            ?>
                                                            </div>
                                                            ...
                                                            <div class="col-8">
                                                                <div class="table-responsive d-flex" style="height: 200px; overflow: auto; overflow-x: auto; overflow-y: auto;">
                                                                    <table class="table table-sm table-hover">
                                                                        <thead>
                                                                            <tr>
                                                                                <th scope="col" style="width: 60%;"><small><small>Action</small></small></th>
                                                                                <th scope="col" style="width: 20%;"><small><small>Name</small></small></th>
                                                                                <th scope="col" style="width: 20%;"><small><small>Date / Time</small></small></th>
                                                                            </tr>
                                                                        </thead>
                                                                        <tbody>
                                                                            <?php
                                                                                $sql_actions = "SELECT action, empNo, date, time FROM risktreatmentresponseaction WHERE indexRisk = '$riskIndex' ORDER BY id DESC";
                                                                ...
                                                                <?php
                                                                    if($risk_details['closeStatus'] == 0){
                                                                ?>
                                                                <button data-bs-toggle="modal" data-bs-target="#Add-Risk-Action" type="button" style="padding: .35rem .5rem; font-size: .900rem; line-height: .95; border-radius: .2rem; float: right;" class="btn btn-info" <?php if((($riskReporter == 0) & ($riskOwnership == 0) & ($actionOwnership == 0)) | ($activeStatus != "Active")){   echo    "disabled";  }  ?>>Add Action</button>
                                                                <?php
                                                                    }
                                                                ?>
                                                            </div>
                                                    ...
                                                    <?php
                                                        if($risk_details['closeStatus'] == 0){
                                                            $actionStatus = array('Abandoned', 'Completed (delay)', 'Completed (early)', 'Completed (on time)');
                                                    ?>
                                                        <button data-bs-toggle="modal" data-bs-target="#Close-Risk" type="button" class="btn btn-sm btn-danger mx-2"<?php if((($riskReporter == 0) & ($riskOwnership == 0) & ($actionOwnership == 0)) | ($activeStatus != "Active") | (!(in_array($risk_details['currentStatus'], $actionStatus)))){   echo    "disabled";  }  ?>>Request to close this Risk</button>
                                                    <?php
                                                        }
                                                    ?>
                                                    </div>
        ...
        <script type="text/javascript">
            ...
            $('#addMeRiskOwner').click(function(){
                if(confirm("Are you sure you want to add you as risk owner?")){
                    $("#addMeRiskOwner").html("Adding...");
                    document.getElementById('addMeRiskOwner').disabled = true;
                    document.body.style.cursor = 'wait';
                    var indexRisk = `<?php   echo    $riskIndex; ?>`;
                    var empNo = `<?php   echo    $userDetails['empNo'];   ?>`;
                    $.ajax({
                        url: "Add-Me-Risk-Owner.php",
                        method: "POST",
            ...
            $('#requestSendDivision').click(function(){
                if(confirm("Are you sure you want to request this risk send to Divisinal Risk Registry?")){
                    $("#requestSendDivision").html("Requesting...");
                    document.getElementById('requestSendDivision').disabled = true;
                    document.body.style.cursor = 'wait';
                    var indexRisk = `<?php   echo    $riskIndex; ?>`;
                    var empNo = `<?php   echo    $userDetails['empNo'];   ?>`;
                    $.ajax({
                        url: "Request-Send-Risk-Division.php",
                        method: "POST",
                        data: {empNo:empNo,indexRisk:indexRisk},
                        dataType: "text",
                        success: function(data){
