<?php
    ...
    if($conn->query($queryInesrtRisk)=== TRUE){
        ...
        if($conn->query($queryRisk)=== TRUE){
            for($count = 0; $count<count($_POST['riskOwner']); $count++){
            ...
            for($countAO = 1; $countAO<count($_POST['Owner']); $countAO++){
                ...
                $slAO = "INSERT INTO actionowners (indexRisk, empNo) VALUES ('$TR','$empAO')";
            ...
            if($conn->query($queryActivity)=== TRUE){
                ...
                if($conn->query($queryResponseAction)=== TRUE){
                    $queryLastUpdate = "INSERT INTO lastupdate (indexRisk,empNo,date,time) VALUES ('$TR','$reporter','$Date','$Time')";
                    if($conn->query($queryLastUpdate)=== TRUE){
                        $queryResActivity = "INSERT INTO activity (indexRisk,empNo,Date,Time,Des) VALUES ('$TR','$reporter','$Date','$Time','Newly Added')";
