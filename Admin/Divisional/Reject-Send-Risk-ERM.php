<?php
    ...
    $comment = $_POST['comment'];
        ...
        $mail->Body   .= 'Admin reject your request to transfer risk <em>'.$indexRisk.'</em> from Divisional Risk Registry to ERM with comment - <em>'.$comment.'</em>.<br>';
        ...
        $queryActivity = "INSERT INTO divactivity (indexRisk, empNo, Date, Time, Des) VALUES ('$indexRisk', '$empNo', '$Date', '$Time', '$com')";
        if($conn->query($queryActivity) === TRUE){
            $queryUpdateTrindex = "UPDATE divtrindex SET status = 'Active' WHERE indexRisk = '$indexRisk'";
